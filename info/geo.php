...
echo '<table class="info"><tbody>';
echo '<tr><td class="label">google_map:</td>';
echo '<td><img src="http://maps.google.com/maps/api/staticmap?center='.$lat.','.$long.'&zoom=14&size=400x400&maptype=roadmap&&sensor=true" width="400" height="400" alt="'.$geodata['formatted_address'].'" \/></td>';
echo '</tr>';

foreach($geodata as $name => $value){
  echo '<tr>';
	echo '<td class="label">'.$name.':</td><td>'.str_replace('&','&amp;',$value).'</td>';
	echo '</tr>';
}
echo '</tbody></table>';
