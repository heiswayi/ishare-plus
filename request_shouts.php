...
<div class="head-text">
  ...
  <h3>Any shouted messages with !request command will be saved here</h3>
</div>
</div>
...
$per_page = 20;
$result = mysql_query("SELECT * FROM i_requests ORDER BY id DESC") or die(mysql_error());
...
echo '</th></tr>';
echo '</thead><tbody>';
echo '<tr><td class="request-desc" colspan="2">Type <span class="requestcode">!request Your_Request_Message</span> on the Shoutbox to make your request message appears here. Only the latest 3 replied messages will be displayed for each request.</td></tr>';
...
for ($i = $start; $i < $end; $i++) {
    ...
    <td>' . $shoutx . '<div class="timeago" id="timeago-' . $userid . '" style="display:none;text-align:right;">';
            if (isset($_SESSION['userwd'])) {
                $lines[] = '<a href="reply.php?id='.$userid.'&uref='.$nick.'" class="label label-success"><i class="icon-retweet icon-white"></i> Reply to this Request</a> ';
            }
            $lines[] = '</div>';
