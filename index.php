...
<div class="head-text">
  <h1 id="airport" class="entry-title">Portal of Sharers</h1>
  <h3>It's not just for sharers, it's for everyone. Sharing something good to people is better than doing nothing!</h3>
</div>
</div>
...
function $(a){return document.getElementById(a)}function urlencode(a){a=(a+"").toString();return encodeURIComponent(a).replace(/!/g,"%21").replace(/'/g,"%27").replace(/\(/g,"%28").replace(/\)/g,"%29").replace(/\*/g,"%2A").replace(/%20/g,"+")}function shouts(){clearTimeout(getshout);var a=window.XMLHttpRequest?new XMLHttpRequest:new ActiveXObject("Microsoft.XMLHTTP");a.open("GET","shouts.php?i="+Math.random(),true);a.onreadystatechange=function(){if(this.readyState==4){if(parseInt(this.responseText)>current_shouts){getshouts();current_shouts=parseInt(this.responseText)}getshout=setTimeout("shouts()",1e3)}};a.send(null)}function getshouts(){var a=window.XMLHttpRequest?new XMLHttpRequest:new ActiveXObject("Microsoft.XMLHTTP");a.open("GET","get_shouts.php?i="+Math.random(),true);a.onreadystatechange=function(){if(this.readyState==4)$("shoutbox-reload").innerHTML=this.responseText};a.send(null)}function push_shout(){shouting();return false}function shouting(){var a=window.XMLHttpRequest?new XMLHttpRequest:new ActiveXObject("Microsoft.XMLHTTP");a.open("POST","post_shout.php",true);var b="user="+urlencode($("user").value)+"&"+"shout="+urlencode($("shout").value)+"&"+"color-text="+urlencode($("select-colors").value)+"&"+"antispam="+urlencode($("antispam").value);a.setRequestHeader("Content-type","application/x-www-form-urlencoded");a.setRequestHeader("Content-length",b.length);a.onreadystatechange=function(){if(this.readyState==4){if(!this.responseText)$("shout").value="";else{$("console").style.display="";$("console").innerHTML=this.responseText;setTimeout("$('console').style.display='none';$('console').innerHTML = ''",3e3)}getshouts()}};a.send(b);return true}var current_shouts=0;var getshout=setTimeout("shouts()",1e3)
</script>

<div id="shoutbox-reload"><?php include('get_shouts.php'); ?></div>
<div class="archived-nav"><a href="more.php?page=2" class="label label-inverse arc-btn-tip" title="Show previous shouted messages">View Archived Messages <i class="icon-chevron-right icon-white"></i></a></div>
</div>
