...
<div class="head-text">
  ...
  <h3>You might be want to update your profile information, or you may change it later! :)</h3>
</div>
</div>
...
if ($userdatax) {
    ...
    if ($fullnamex) { echo '<td>'.$fullnamex.'</td>'; } else { echo '<td style="background:#FFFF6B">Your name here</td>'; }
    echo '</tr>';
    echo '<tr>';
    ...
    if ($freenotex) { echo '<td>'.linkThisOne(stripslashes(rtrim($freenotex))).'</td>'; } else { echo '<td style="background:#FFFF6B">Write anything you want. And this is a sample URL/link: '.linkThisOne('http://www.google.com').'</td>'; }
    echo '</tr>';
}
...
if ($sharerdatax) {
    ...
    echo '<td><span class="label label-info">'.$sharernamex.'</span></td>';
    echo '</tr>';
    echo '<tr>';
    echo '<td class="user-info-label">Sharer link</td>';
    echo '<td><a href="'.$sharerlinkx.'">'.$sharerlinkx.'</a></td>';
    echo '</tr>';
    echo '<tr>';
    echo '<td class="user-info-label">Sharer description</td>';
    echo '<td>'.stripslashes(rtrim($sharerdescx)).'</td>';
    echo '</tr>';
} else {
    echo '<tr>';
    echo '<td colspan="2" style="background:#336699;color:#fff;"><i class="icon-exclamation-sign icon-white"></i> If you are a sharer, you may place your sharerlink information here too! <i class="icon-hand-down icon-white"></i></td>';
    echo '</tr>';
    echo '<tr>';
    echo '<td class="user-info-label">Sharer name</td>';
    echo '<td style="background:#eee"><span class="label label-info">Your sharer name</span></td>';
    echo '</tr>';
    echo '<tr>';
    echo '<td class="user-info-label">Sharer link</td>';
    echo '<td style="background:#eee"><a href="#">http://your_ip:port/</a></td>';
    echo '</tr>';
    echo '<tr>';
    echo '<td class="user-info-label">Sharer description</td>';
    echo '<td style="background:#eee">Your sharer description</td>';
    echo '</tr>';
}
...
<div class="well"><a class="btn btn-inverse" href="index.php">Just Skip to Shoutbox <i class="icon-chevron-right icon-white"></i></a></div>
  ...
  <legend>Update Your Profile</legend>
    <div class="control-group">
      ...
      <div class="controls">
        ...
        <p class="help-inline"><span class="label label-info regtip-info" title="Once registered, you cannot change your username anymore."><i class="icon-exclamation-sign icon-white"></i></span></p>
      </div>
    </div>
    <div class="control-group">
      <label class="control-label" for="fullname">Name</label>
      <div class="controls">
        <input type="text" class="input-large regtip" id="fullname" name="fullname" value="<?php echo $fullname; ?>" maxlength="30" title="Max. 30 chars including alphanumerics, whitespaces and dots." style="background:#FFFF6B">
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <input type="text" class="input-large regtip" id="tagline" name="tagline" value="<?php echo $tagline; ?>" maxlength="100" title="Max. 100 chars. Be short and simple." style="background:#FFFF6B">
        <p class="help-inline"><span class="label label-success"><i class="icon-question-sign icon-white"></i> Optional</span></p>
      </div>
    </div>
    <div class="control-group">
      ...
      <div class="controls">
        <textarea class="input-large regtip" id="freenote" rows="5" placeholder="Write something." name="freenote" maxlength="500" title="Max. 500 chars including whitespaces, alphanumerics and symbols." style="background:#FFFF6B"><?php echo $freenote; ?></textarea>
        <p class="help-inline"><span class="label label-success"><i class="icon-question-sign icon-white"></i> Optional</span></p>
      </div>
    </div>
    ...
    <div class="control-group">
      ...
      <div class="controls">
        <textarea class="input-large regtip" id="sharerdesc" placeholder="Be simple & descriptive..." rows="3" name="sharerdesc" maxlength="200" title="Max. 200 chars including whitespaces, alphanumerics and symbols."><?php echo $sharerdesc; ?></textarea>
        <p class="help-inline"><span class="label label-success"><i class="icon-question-sign icon-white"></i> Optional</span></p>
      </div>
    </div>
    <div class="control-group">
      ...
      <div class="controls">
        ...
        <a class="btn btn-inverse" href="index.php">Skip to Shoutbox</a>
      </div>
    </div>
  </fieldset>
</form>
...
if (isset($_POST['update'])) {
    ...
    $sharerdesc     = stripslashes(rtrim(mysql_real_escape_string(htmlspecialchars($_POST['sharerdesc']))));
    $freenote       = stripslashes(rtrim(mysql_real_escape_string(htmlspecialchars($_POST['freenote']))));
    $fullname       = mysql_real_escape_string(htmlspecialchars($_POST['fullname']));
    $tagline        = stripslashes(rtrim(mysql_real_escape_string(htmlspecialchars($_POST['tagline']))));
    ...
    if ($email == '') {
        ...
        renderForm($username, $error, $email, $sharername, $sharerlink, $sharerdesc, $freenote, $fullname, $tagline);
    } else if ($sharername !== '' && $sharerlink == '') {
        $error = '<strong>ERROR:</strong> Please insert your sharerlink.';
        renderForm($username, $error, $email, $sharername, $sharerlink, $sharerdesc, $freenote, $fullname, $tagline);
    } else if ($sharerlink !== '' && $sharername == '') {
        $error = '<strong>ERROR:</strong> You need to insert your sharer name too!';
        renderForm($username, $error, $email, $sharername, $sharerlink, $sharerdesc, $freenote, $fullname, $tagline);
    } else if ($fullname !== '' && !validateFullname($fullname)) {
        $error = '<strong>ERROR:</strong> Only alphanumerics, whitespaces and dots are allowed!';
        renderForm($username, $error, $email, $sharername, $sharerlink, $sharerdesc, $freenote, $fullname, $tagline);
    } else if ($sharerdesc !== '' && !check_sharername($sharerdesc)) {
        $error = '<strong>ERROR:</strong> Only alphanumerics, dot, underscore and whitespace are allowed!';
        renderForm($username, $error, $email, $sharername, $sharerlink, $sharerdesc, $freenote, $fullname, $tagline);
    } else {
        mysql_query("UPDATE i_users SET email='$email', free_note='$freenote', userip='$ip', fullname='$fullname', tagline='$tagline' WHERE userwd='$username'") or die(mysql_error());
        ...
        if ($initSharerData) {
            ...
            mysql_query("UPDATE i_sharerlinks SET sharername='$sharername', sharerlink='$sharerlink', sharerdesc='$sharerdesc', add_date='$add_date' WHERE owner='$owner'") or die(mysql_error());
        } else {
            $likes = 0;
            mysql_query("INSERT i_sharerlinks SET owner='$owner', sharername='$sharername', sharerlink='$sharerlink', sharerdesc='$sharerdesc', add_date='$add_date', likes='$likes'") or die(mysql_error());
        }
        }
